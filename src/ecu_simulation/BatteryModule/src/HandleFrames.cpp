...
void HandleFrames::processReceivedFrame(const struct can_frame &frame) {
    ...
    bool isSingleFrame = !isFirstFrame && !isConsecutiveFrame;
    ...
    if (isFirstFrame) {
        ...
        stored_data = std::vector<int>(data.begin(), data.end()); // Store initial data including PCI byte
        expected_data_size = ((data[1] & 0x0F) << 8) + data[2]; // Expected total size of data
        flag = (expected_data_size + 6) / 7; // Calculate the number of consecutive frames expected
    ...
    // Handle Single Frames
        ...
        // std::cout << "Single frame received\n";
    ...
    if (flag == 0) {
        if (isFirstFrame || isConsecutiveFrame) {
            std::cout << "\nComplete multi-frame message received\n";
        } else {
...
void HandleFrames::handleCompleteData(int id,const std::vector<int>& data, bool isSingleFrame) {
    // Extract the Service Identifier (SID)
    //int sid = data[1]; // SID is always at data[1]
