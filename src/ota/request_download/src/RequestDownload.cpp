...
void RequestDownloadService::requestDownloadRequest(int id, std::vector<uint8_t> stored_data, Logger &RDSlogger, DiagnosticSessionControl mcuDiagnosticSessionControl, ReadDataByIdentifier software_version, SecurityAccess logged_in)
{
    ...
    if (!isRequestAuthenticated(logged_in, RDSlogger))
    {
        ...
        /* sendNegativeResponse(id, 0x34, 0x34); // Not in programming session, authentication required*/
        return;
    }
    ...
    {
        // Handle case where software is not at the latest version
        LOG_INFO(RDSlogger.GET_LOGGER(), "Software is not at the latest version");
        id = (frame_sender_id << 8) | frame_dest_id;
        /* sendNegativeResponse(id, 0x34, 0x33); // Security access denied */
        return;
    }
    ...
    //std::vector<uint8_t> payload_compr = processPayload(stored_data, compression_type, 4, RDSlogger);
    ...
    if (stored_data.size() < static_cast<std::vector<unsigned char>::size_type>(1 + length_memory_address + length_memory_size))
    {
        ...
        /* sendNegativeResponse(id, 0x34, 0x13); // Incorrect message length or invalid format*/
    }
    ...
    if (position_download_type < stored_data.size()) 
    {
        ...
        // Optionally, you can validate the download_type value here
        if (download_type != 0x88 && download_type != 0x89) {
            ...
            // Handle the error or return false indicating invalid data
        }
    }  
    ...
    if (!isValidMemoryRange(memory_address, memory_size, RDSlogger))
    {
        ...
        /* sendNegativeResponse(id, 0x34, 0x31); // Request out of range*/
        return;
    }

    /* Assuming 512 bytes as the max length, adjust this based on server capabilities */
    int block_size = 512;
...
bool RequestDownloadService::isValidMemoryRange(const std::vector<int> &memory_address, const std::vector<int> &memory_size, Logger &RDSlogger)
{
    ...
    if (full_memory_address < 0 || (static_cast<unsigned int>(full_memory_address) > 0xFFFFFFFFU))
    {
        ...
        /* sendNegativeResponse(id, 0x34, 0x31); // Request out of range -Invalid memory address*/
        return false;
    }
    ...
    if (full_memory_size < 1)
    {
        ...
        /* sendNegativeResponse(id, 0x34, 0x31); // Request out of range -Invalid memory size*/
        return false;
    }
