...
void RequestDownloadService::requestDownloadRequest(canid_t id, std::vector<uint8_t> stored_data)
{
    ...
    auto ota_status = FileManager::getDidValue(OTA_UPDATE_STATUS_DID)[0];
    ...
    FileManager::setDidValue(OTA_UPDATE_STATUS_DID, {WAIT}, aux_can_id, socket, RDSlogger);
    ...
    if (!isValidMemoryRange(memory_address, memory_size))
    {
        ...
        FileManager::setDidValue(OTA_UPDATE_STATUS_DID, {WAIT_DOWNLOAD_FAILED}, aux_can_id, socket, RDSlogger);
        AccessTimingParameter::stopTimingFlag(receiver_id, 0x34);
        return;
    }
    ...
    FileManager::setDidValue(OTA_UPDATE_STATUS_DID, {WAIT_DOWNLOAD_COMPLETED}, aux_can_id, socket, RDSlogger);
