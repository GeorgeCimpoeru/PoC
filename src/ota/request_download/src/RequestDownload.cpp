...
void RequestDownloadService::requestDownloadRequest(int id, std::vector<uint8_t>stored_data, Logger& RDSlogger) {
    ...
    if (compression_type == 0x0) {
        ...
        // Example: Decompress using Zlib
        // Decompress stored_data from position 4 onwards (assuming position 2 is DFI)
        //std::vector<uint8_t> decompressed_data = decompressZlib(stored_data, 2);
    ...
    uint8_t length_max_number_block;
...
void RequestDownloadService::requestDownloadResponse(int id, uint8_t length_max_number_block, int max_number_block, Logger &RDSlogger)
{
    ...
    LOG_INFO(RDSlogger.GET_LOGGER(), "frame id dest: {}", static_cast<int>(frame_dest_id));
    if(frame_dest_id == 0x10)
    {
        ...
        if (create_interface)
        {
            ...
            int sid = 0x74;
            //length_max_number_block 
            //max_number_block
            generateFrames.sendFrame(id, stored_data, socket_fd, DATA_FRAME);
            return;
        }
        else{
...
bool RequestDownloadService::isValidMemoryRange(const std::vector<int>& memory_address, const std::vector<int>& memory_size, Logger &RDSlogger) {
    // Logic to validate the memory address and size 
    if (memory_address.size() != memory_size.size()) {
        LOG_ERROR(RDSlogger.GET_LOGGER(), "Error: Memory address and size counts do not match");
        return false;
    }

    const int maxMemoryAddress = 0xFFFFFFFF; 
    const int minMemorySize = 1;        

    for (size_t i = 0; i < memory_address.size(); ++i) {
        int address = memory_address[i];
        int size = memory_size[i];

        // Validate memory address
        if (address < 0 || address > maxMemoryAddress) {
            LOG_ERROR(RDSlogger.GET_LOGGER(), "Error: Invalid memory address: 0x{0:x}", address);
            return false;
        }

        // Validate memory size
        if (size < minMemorySize) {
            LOG_ERROR(RDSlogger.GET_LOGGER(), "Error: Invalid memory size: 0x{0:x}",size);
            return false;
        }
