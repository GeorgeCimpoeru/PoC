package com.poc.p_couds.ui.theme
...
class UDSactivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            CarsDataTheme {
                ActivityLayout {
                    // Only replace the fragment if it has not been added yet
                    if (isFragmentAdded) {
                        replaceFragment(Fragment_Update())
                        isFragmentAdded = true // Update the state to avoid re-adding
                    }
                }
            }
        }
    ...
    @Composable
    fun ActivityLayout(onFragmentCreated: (View) -> Unit) {
        ...
        Scaffold(
            ...
            Column(
                ...
                // Fragment container using AndroidView
                AndroidView(factory = { context ->
                    FrameLayout(context).apply {
                        id = R.id.fragment_container // Set the ID for the fragment container
                    }
                }, update = { view ->
    ...
    @Composable
    fun TopBarNavigation(navController: NavController) {
    ...
    fun ActivPreview() {
        CarsDataTheme {
            ActivityLayout { fragmentContainer ->
                // This block will be executed once the fragmentContainer is ready
                if (!isFragmentAdded) {
                    replaceFragment(Fragment_Update())
                    isFragmentAdded = true // Update the state to avoid re-adding
                }
